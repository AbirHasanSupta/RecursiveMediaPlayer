...
class VLCPlayerController:
    ...
    def play_video(self, index):
        with self.lock:
            ...
            # Try setting fullscreen multiple times to enforce it
            for _ in range(5):
    ...
    def toggle_fullscreen(self):
        with self.lock:
            current_state = self.player.get_fullscreen()
            self.player.set_fullscreen(not current_state)
            print(f"Fullscreen set to {not current_state}")
...
def listen_keys(controller):
    ...
    keyboard.wait('esc')  # keep listener alive until Esc pressed
