...
def select_multiple_folders_and_play():
    ...
    class DirectorySelector(ThemeSelector):
        ...
        def _setup_app_keyboard_navigation(self):
            ...
            self.root.bind("<Control-O>", lambda e: self._add_directory_shortcut())
            self.root.bind("h", self._go_home_shortcut)
            self.root.bind("H", self._go_home_shortcut)
            ...
            self.root.bind("<F10>", self._open_context_menu_for_focused)
            self.root.bind("<period>", self._open_context_menu_for_focused)
            self.root.bind("v", self._show_preview_shortcut)
            self.root.bind("V", self._show_preview_shortcut)
            self.root.bind("<Control-k>", self._focus_global_search)
            self.root.bind("<Control-K>", self._focus_global_search)
        ...
        def _go_home_shortcut(self, event=None):
        ...
        def _focus_global_search(self, event=None):
            if not self._shortcuts_allowed():
                return "break"
            entry = getattr(self, 'global_search_entry', None)
            if entry and entry.winfo_exists() and entry.winfo_ismapped():
                entry.focus_set()
